...
def trapezoidalsingleMoveClose(position, reply_enable, server_ip, motor_number, accuracy_threshold):
    trapezoidalMove(position,reply_enable,server_ip,motor_number)
    while True:
        cvp = aios.getCVP(server_ip, 1)
        if abs(cvp[0] - position) <= accuracy_threshold:
            break


def trapezoidalMoveClose(position_dict, reply_enable, server_ips, motor_number, accuracy_threshold):

    for i in range(len(server_ips)):
        trapezoidalMove(position_dict[server_ips[i]],reply_enable,server_ips[i],motor_number)
    # 对梯形插补进行闭环
    try:
        start = time.time()
        while True:
            break_flag = True
            for i in range(len(server_ips)):
                cvp = aios.getCVP(server_ips[i], 1)
                if abs(cvp[0] - position_dict[server_ips[i]]) > accuracy_threshold:
                    break_flag = False
            if break_flag:
                break
