...
class RPCServer(object):
    def __init__(self, ):
        ...
        for ip in Server_IP_list:
            data = {"method": "GET","reqTarget": "/"}
            json_str = json.dumps(data)
            print("Send JSON Obj:", json_str)
            aios.s.sendto(str.encode(json_str), (ip, 2334))
            try:
                data, address = aios.s.recvfrom(1024)
                print('Server received from {}:{}'.format(address, data.decode('utf-8')))
                json_obj = json.loads(data.decode('utf-8'))
                if 'deviceType' in json_obj:
                    if json_obj['deviceType'] == 'ctrlbox':
                        self.ctrlbox_ip = ip
    ...
    def do_step(self):
        ...
        while True:
            ...
            mv_data = revc_data[0].decode('utf-8')
            for key, val in self.mv_data.items():
    ...
    def em_stop(self):
        ...
        self.pause()
    def hw_stop(self):
        aios.shutdown(self.ctrlbox_ip)
    def hw_open(self):
        aios.openup(self.ctrlbox_ip)
    def back_start(self):
        pass
    ...
    def delay_err(self):
        ...
        for i in sub_ips:
            ...
            aios.s.sendto(str.encode(json_str), (ip, 2334))
            try:
                data_rev, address = aios.s.recvfrom(1024)
                data_rev = json.loads(data_rev.decode('utf-8'))
            except socket.timeout:  # fail after 1 second of no activity
                print("Didn't receive anymore data! [Timeout]")
            ...
            return {'delay_time': delay_time, 'err_msg':data_rev}
